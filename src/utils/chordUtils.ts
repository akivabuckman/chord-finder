...
export const chordFinderAlgorithm = (songKey: string, melodyNote: string): AlgorithmReturnType => {
    ...
    const romanChordOptions: RomanChordOptionsType = {
        standard: romanChords.standard.map(romanChord => romanChordOptionsFromInterval[interval.numeric].includes(romanChord) ? romanChord : null).filter(romanChord => romanChord !== null),
        funky: romanChords.funky.map(romanChord => romanChordOptionsFromInterval[interval.numeric].includes(romanChord) ? romanChord : null).filter(romanChord => romanChord !== null),
        superFunky: romanChords.superFunky.map(romanChord => romanChordOptionsFromInterval[interval.numeric].includes(romanChord) ? romanChord : null).filter(romanChord => romanChord !== null)
    };
